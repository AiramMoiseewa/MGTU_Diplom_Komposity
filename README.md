...
6).	Написано 5 нейронных сетей, которые будут рекомендовать соотношение "матрица-наполнитель".
...
В ходе исследования было доказано, что взаимосвязь между переменными есть, но из-за слабой корреляции точность прогноза не высока. Полученный результат является лишь шаблоном для создания реальной модели прогнозирования. Если получить доступ к большему объему информации, есть вероятность, что прототип приложения будет выдавать лучшие результаты. 

Структура репозитория:

ML_FLASK
Diplom_Moiseeva_MA_ipynb_.ipynb
X_bp.xlsx
X_nup.xlsx
Пояснительная записка Моисеева М.А..docx
Пояснительная записка Моисеева М.А..pdf
Презентация_МГТУ_Моисеева М.А.pdf
Презентация_МГТУ_Моисеева М.А.pptx
